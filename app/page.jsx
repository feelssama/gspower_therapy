...
const GSLogo = ({ size = 56, onClick }) => (
  <div onClick={onClick} className={`flex items-center gap-1.5 ${onClick ? 'cursor-pointer active:scale-95 transition-transform' : ''}`}>
    <div className="flex items-baseline gap-[2px] ml-0.5">
      <span className="font-black tracking-tighter" style={{ color: '#2B4C8C', fontSize: size * 0.65 }}>GS</span>
      <span className="font-bold tracking-tight text-[#555555]" style={{ fontSize: size * 0.6 }}>파워</span>
    </div>
  </div>
);
...
export default function TherapyApp() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (uData) setRegisteredUsers(uData.map(u => ({ name: u.name, empId: String(u.emp_id || '').toUpperCase() })));
  ...
  const handleRate = async (pid, rate) => {
    await supabase.from('programs').update({ rating: rate }).eq('id', pid);
  ...
  const filtered = programs.filter(p => (filterLoc === '전체' || p.location.includes(filterLoc)) && (!searchQ || p.title.includes(searchQ)));
  const colorMap = {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-6 bg-[#FAFAF7]">
        ...
        <div className="w-full max-w-[400px] a-slide-up text-center">
          <div className="mb-10 inline-block bg-white p-6 rounded-3xl shadow-sm"><GSLogo size={50} /></div>
          <h1 className="text-[32px] font-black leading-tight mb-8 text-[#0A1628]">건강한 당신이<br/><span className="text-[#2B4C8C]">곧 건강한 회사입니다</span></h1>
      ...
      <main className="flex-1 p-5 lg:p-12 pb-24 overflow-x-hidden">
        ...
        {currentTab === 'home' && (
          <div className="space-y-8 a-fade">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 stagger">
              ...
              <StatCard icon={TrendingUp} label="총 신청 인원" value={programs.reduce((a,p)=>a+(p.applied||0),0)} suffix="명" color="#F47B20" />
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {programs.slice(0, 2).map(p => <FeaturedCard key={p.id} program={p} colorMap={colorMap} onClick={()=>{setSelectedProgram(p);setShowDetail(true);}} />)}
            </div>
          </div>
        )}
        ...
        {currentTab === 'programs' && (
          <div className="space-y-8 a-fade">
            <div className="flex flex-col gap-4">
              <div className="relative"><Search size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-300" /><input value={searchQ} onChange={e=>setSearchQ(e.target.value)} placeholder="검색어를 입력하세요" className="w-full bg-white border border-gray-100 rounded-3xl pl-12 py-5 font-black outline-none" /></div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 stagger">
              {filtered.map(p => <CompactCard key={p.id} program={p} colorMap={colorMap} onClick={()=>{setSelectedProgram(p);setShowDetail(true);}} />)}
            </div>
          </div>
        )}
        ...
        {currentTab === 'my' && (
          <div className="space-y-6 a-fade">
            ...
            {myApplications.length === 0 ? <div className="p-20 text-center text-gray-300 font-black bg-white rounded-3xl border border-gray-100">신청 내역이 없습니다</div> : (
              <div className="space-y-4">
                {myApplications.map((p, i) => (
                  <div key={i} className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
                    <div className="flex justify-between items-start mb-4">
                      <div className="flex items-center gap-3"><div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${colorMap[p.color]?.bg || 'bg-gray-100'}`}><Stethoscope style={{color: colorMap[p.color]?.solid}}/></div><div><h3 className="font-black text-[17px]">{p.title}</h3><p className="text-[12px] text-gray-400 font-bold">{p.location} · {formatDate(p.date)}</p></div></div>
                    ...
                    <div className="bg-gray-50 p-4 rounded-2xl flex flex-col md:flex-row items-center justify-between gap-4 mt-4">
                      <div className="flex items-center gap-2 text-[12px] font-black text-gray-600"><ThumbsUp size={14} className="text-orange-500"/> 만족도를 평가해주세요!</div>
        ...
        {currentTab === 'admin' && <AdminPanel programs={programs} users={registeredUsers} onLottery={handleLottery} colorMap={colorMap} />}
      </main>
  ...
  return (
    <button onClick={onClick} className="w-full text-left bg-white p-7 rounded-[2rem] border border-gray-100 shadow-sm relative overflow-hidden hover:shadow-xl transition-all group">
      ...
      <div className="relative">
        <div className="flex justify-between mb-8"><span className={`text-[10px] font-black px-3 py-1 rounded-full ${c.bg} ${c.text}`}>{program.category}</span><StatusBadge status={getProgramStatus(program)}/></div>
...
const AdminPanel = ({ programs, users, onLottery, colorMap }) => {
  ...
  const inputCls = "w-full bg-gray-50 border-2 border-transparent rounded-2xl px-5 py-4 font-black outline-none focus:bg-white focus:border-black transition-all";
  ...
  return (
    <div className="space-y-12 pb-20">
      ...
      {lotteryResult && <div className="fixed inset-0 bg-black/70 flex items-center justify-center p-6 z-[200] a-fade"><div className="bg-white p-8 rounded-3xl text-center"><Dna size={40} className="mx-auto mb-4 text-blue-500"/><h3 className="font-black mb-4">{lotteryResult}</h3><button onClick={()=>setLotteryResult(null)} className="bg-black text-white px-8 py-3 rounded-xl font-bold">확인</button></div></div>}
    </div>
  );
};
